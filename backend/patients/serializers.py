...
class PatientOverviewItemSerializer(serializers.Serializer):
    """Response item for GET /api/patients/overview/"""
...
def _validate_assignable_doctor(value):
    """Shared validation: doctor exists, has doctor role, and is active."""
    from authentication.models import CustomUser
    try:
        doctor = CustomUser.objects.get(id=value)
    except CustomUser.DoesNotExist:
...
class AdminPatientListSerializer(serializers.ModelSerializer):
    """Read serializer for the admin center-wide roster (one row per patient)."""

    assigned_doctor = serializers.SerializerMethodField()

    class Meta:
        model = Patient
        fields = ['id', 'full_name', 'date_of_birth', 'contact_email',
                  'created_at', 'assigned_doctor']
        read_only_fields = fields

    def get_assigned_doctor(self, obj):
        """Return the most-recent assigned doctor as {id, name, email}, or None."""
        # doctor_assignments default ordering is -assigned_at, so first() is newest.
        assignment = obj.doctor_assignments.all().first()
        if not assignment:
            return None
...
class AdminPatientRegisterSerializer(serializers.Serializer):
    """Write serializer: admin registers a patient and optionally assigns a doctor."""
    ...
    doctor_id = serializers.IntegerField(required=False, allow_null=True, write_only=True)
    ...
    def validate_doctor_id(self, value):
        if value in (None, ''):
            return None
        return _validate_assignable_doctor(value)
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            patient = Patient.objects.create(created_by=request.user, **validated_data)
            if doctor_id:
                doctor = CustomUser.objects.get(id=doctor_id)
                DoctorPatientAssignment.objects.create(
                    doctor=doctor,
                    patient=patient,
                    assigned_by=None,  # admin is not a doctor; clean() skips role check when None
                )
        return patient

    def to_representation(self, instance):
        return AdminPatientListSerializer(instance, context=self.context).data


class AdminPatientAssignSerializer(serializers.Serializer):
    """Write serializer for assigning/reassigning a patient to a single doctor."""
