"""
1D-CNN Model Training Script

Trains a 1D-CNN model with 3 Conv1D layers + BatchNorm + MaxPool for
Parkinson's tremor detection. Exports trained model and metadata.
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Train 1D-CNN classifier for tremor detection')
    parser.add_argument(
        '--input-dir',
        type=str,
...
def main():
    ...
    # Set random seeds for reproducibility (T042)
    ...
    # TensorFlow version and GPU detection (T043)
    ...
    try:
        ...
        # Step 2: Validate data (T032)
        validate_data(X_train, y_train, X_test, y_test)

        # Step 3: Split training data into train/validation (T033)
        ...
        # Step 4: Build 1D-CNN model (T034)
        ...
        logger.info("  - Conv1D(64, kernel=3) + BatchNorm + MaxPool(2)")
        logger.info("  - Conv1D(128, kernel=3) + BatchNorm + MaxPool(2)")
        logger.info("  - Conv1D(256, kernel=3) + BatchNorm + MaxPool(2)")
        logger.info("  - Flatten + Dense(128) + Dropout(0.5) + Dense(1, sigmoid)")
        logger.info(f"Total parameters: {model.count_params():,}")

        # Step 5: Setup early stopping (T035)
        ...
        # Step 7: Evaluate on test set (T037)
        ...
        model_path, metadata_path = save_model(
            ...
            model_name="cnn_1d_model"
        )
        logger.info(f"Model saved: {model_path}")
        logger.info(f"Metadata saved: {metadata_path}")

        # Step 11: Validate model loading (T041)
        ...
        logger.info(f"1D-CNN model ready for deployment")
        logger.info("="*70)
