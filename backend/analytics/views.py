...
from analytics.serializers import StatisticsResponseSerializer, DashboardStatsSerializer, CriticalAlertsSerializer
...
class AdminStatsView(APIView):
    ...
    Feature 046: Admin Global Overview

    Returns center-wide aggregate statistics for admin-role users:
    - total_doctors: count of all CustomUser records with role='doctor'
    - total_patients: count of all Patient records in the system

    Access Control:
        - Admin only

    Returns:
        200: { "total_doctors": N, "total_patients": M }
...
class DashboardStatsView(APIView):
    ...
    Returns summary statistics for the logged-in doctor:
...
class CriticalAlertsView(APIView):
    ...
    def get(self, request):
        """Return critical alerts count for the authenticated doctor."""
        if request.user.role != 'doctor':
            return Response(
